...
Zapisuje:
  - wyniki/era5_comparison_summary.csv  (krótkie PL nagłówki)
...
def run():
    ...
    for prov in reqs:
        ...
        if not rows:
            print(f"  No rows for {prov}."); continue
    ...
    if not provider_files:
        ...
        # ale i tak zapisz pusty summary (spójność pipeline'u)
        (outdir / "era5_comparison_summary.csv").write_text("", encoding="utf-8")
        return
    ...
    if args.grid_step and args.grid_step > 0:
        def snap(x, s): return np.round(x / s) * s
    ...
    if merged.empty:
        ...
        # zapisz pusty summary (nagłówki) dla spójności
        cols = ["provider (API)","zmienna","n_api","n_era5","pokrycie%","dorobione%","bias","MAE","RMSE","corr","%zawyż","%zaniż"]
        pd.DataFrame(columns=cols).to_csv(outdir / "era5_comparison_summary.csv", index=False)
        print(f"[OK] Wrote summary: {outdir/'era5_comparison_summary.csv'}")
    ...
    for prov, dfp in m2[m2["variable"]=="precipitation"].groupby("provider"):
        if dfp.empty:
            ev_rows.append({"provider": prov, "POD": np.nan, "FAR": np.nan, "CSI": np.nan}); continue
        pred_ev = (dfp["pred_value"] > thr).to_numpy()
        era_ev  = (dfp["era5_value"] > thr).to_numpy()
        TP = int(np.sum(pred_ev & era_ev)); FP = int(np.sum(pred_ev & ~era_ev)); FN = int(np.sum(~pred_ev & era_ev))
    ...
    for (prov, var), df in m2.groupby(["provider","variable"]):
        ...
        if not np.isnan(sl):
            xx = np.linspace(mn, mx, 50); yy = sl*xx + itc
    ...
    def describe_pattern(row, precip_ev=None):
        var = row["variable"]
        bias = row["bias"]; mae=row["MAE"]; rmse=row["RMSE"]; sl=row["slope"]; itc=row["intercept"]; r2=row["R2"]
        amp=row["diurnal_amp"]; h=row["diurnal_peak_hour"]
        parts = []
        ...
        if var=="precipitation" and precip_ev is not None:
            POD=precip_ev.get("POD"); FAR=precip_ev.get("FAR"); CSI=precip_ev.get("CSI")
    ...
    if not ev_df.empty and ev_df["CSI"].notna().any():
        ...
        for i, r in ev_rank.iterrows():
            index_lines.append(f"{i+1}. **{r['provider']}** — CSI={r['CSI']:.2f}, POD={r['POD']:.2f}, FAR={r['FAR']:.2f}")
        index_lines.append("")

    import re as _re
    for prov in providers_u:
        p_df = patterns[patterns["provider"]==prov].copy()
        ev_row = ev_df[ev_df["provider"]==prov].iloc[0].to_dict() if (not ev_df.empty and (ev_df["provider"]==prov).any()) else {}
        slug = _re.sub(r"[^a-z0-9]+", "-", prov.lower()).strip("-")
        md_path = analysis_dir / f"provider_{slug}.md"
        lines = [f"# {prov}\n"]
        cols_show = ["variable","pokrycie%","dorobione%","bias","MAE","RMSE","slope","intercept","R2","diurnal_amp","diurnal_peak_hour"]
