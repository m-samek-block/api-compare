...
Co robi:
- skanuje katalog z wynikami (domyślnie ./wyniki/summaries)
...
def load_summaries(root: Path) -> pd.DataFrame:
    # ZMIANA: szuka w folderze summaries/ jeśli istnieje
    summaries_dir = root / "summaries"
    search_dir = summaries_dir if summaries_dir.exists() else root

    files = sorted(list(search_dir.glob("era5_comparison_summary*.csv")))
    if not files:
        # Spróbuj też w głównym folderze, jeśli nie ma w summaries/
        if search_dir != root:
            files = sorted(list(root.glob("era5_comparison_summary*.csv")))

        if not files:
            raise SystemExit(f"Brak plików era5_comparison_summary*.csv w {search_dir} ani w {root}")

    print(f"[INFO] Znaleziono {len(files)} plików summary w {search_dir}")
...
def make_report(all_df: pd.DataFrame, stats: pd.DataFrame, plots_dir: Path, outpath: Path):
    ...
    bias_patterns = (
        stats.assign(trend=lambda r: np.where(r["bias_med"] > 0, "zawyża", np.where(r["bias_med"] < 0, "zaniża", "≈0")))
    ...
    for var, st in bias_patterns.groupby("variable"):
        ...
        for _, r in st.iterrows():
            lines.append(
                f"| {r['provider']} | {r['bias_med']:.3f} | {r['bias_mean']:.3f} | {r['trend']} | {r['slope_bias']:.4f} |")
...
def main():
    ...
    ap.add_argument("--summaries-dir", type=str, default="wyniki",
                    help="Katalog z plikami era5_comparison_summary*.csv (sprawdzi summaries/ lub główny folder)")
